...
if len(idxs) > 0:
    for i in idxs.flatten():
        ...
        thickness = max(3, w0//350)
        font_scale = max(0.7, w0/1400)

        text = f"{DISPLAY_NAMES[label]} {int(conf*100)}%"
        (tw, th), bl = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX,
                                       font_scale, thickness)

        ty = y - 8 if y - 8 > th else y + th + 8

        cv2.rectangle(final_img, (x, ty-th-bl),
                      (x+tw+6, ty+bl), color, -1)
        cv2.rectangle(final_img, (x, y),
...
print(f"📊 Updated metadata: {csv_out}")
